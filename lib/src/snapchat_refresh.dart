...
class SnapchatRefreshState extends State<SnapchatRefresh>
  ...
  void initState() {
    ...
    _progressingController = AnimationController(
        vsync: this, duration: Duration(milliseconds: widget.springAnimationDurationInMilliseconds));
    ...
    switch (_mode) {
      case _LiquidPullToRefreshMode.done:
        ...
        _ringDisappearController.animateTo(1.0,
            duration: Duration(
                milliseconds:
                    (widget.dismissDuration / widget.animSpeedFactor).round()),
            curve: Curves.linear);
        ...
        _radiusController.animateTo(0.0,
            duration: Duration(
                milliseconds:
                    (widget.dismissDuration / (widget.animSpeedFactor * 5))
                        .round()),
            curve: Curves.linear);
...
class CurveHillClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    if (size.height >= centreHeight) {
      // Prevent curveHeight from being larger than available space
      ...
      // Adjusting control points of the curves to avoid the circular shape
      path.quadraticBezierTo(size.width / 4, centreHeight + curveHeight,
          (size.width / 2) - (0 / 2), centreHeight + curveHeight);
